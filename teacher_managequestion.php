...
if(isset($_POST['submit'])){
    extract($_POST);
    $q="INSERT INTO `questions` VALUES(NULL,'$eid','$tid','$sid','$qust','1')";
    $question_id=insert($q);
    //  $noofoption;
    ...
    // alert($status);
      $q="INSERT INTO `option` VALUES(NULL,'$question_id','$t1','$correctanswer')";
      insert($q);
...
<br><br>
     <table class="table" style="width: 700px;">
         ...
         <?php 
            ...
            $qu1="SELECT * FROM `questions` INNER JOIN `option` USING(`question_id`) WHERE `exam_id`='$eid' AND `correctanswer`='Yes'";
            $rt=select($qu1);
            ...
            foreach($rt as $rows){ ?>
                ...
                <?php
              $qu="SELECT * FROM `option` INNER JOIN `questions` USING(`question_id`) WHERE `question_id`=".$rows['question_id']."";
            $ress=select($qu);
         ...
         <?php 
                       if($rows['correctanswer']=='Yes'){ ?>
                     
                        <td ><?php echo $rows['option']; ?></td>
                        <td ><?php echo $rows['maximum_mark']; ?></td>
                    <?php   }
